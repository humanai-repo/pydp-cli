...
def build_parser():
	...
	parser.add_argument('-c', '--column', required=True, help='The column to aggregate.')
	parser.add_argument('--id', help='Column containing a unique identifier to be differentially private with respect to.')
	parser.add_argument('-a', '--aggregation', required=True, action='append', choices=['mean', 'sum'],
		help='Select the required aggregation algorithm.')
	parser.add_argument('-e', '--epsilon', default=1.0, type=float,
		help='Epsilon value for the differential privacy algo. Suggested value is 0.1 - 1.0.')
	parser.add_argument('-o', '--output',
		help='Output file (otherwise writes to STD out).')
	return parser

# Function to read a list of CSVs with headers into a dataframe.
def read_inputs(inputs):
	# TODO: If input not specified then read from standard in.
	frames = [pd.read_csv(i) for i in inputs]
	df = pd.concat(frames)
	return df

# Function given a dataframe and an id column valadates every ID appears exactly once. Throw an error if not.
...
def run_aggregations(df, column, aggregations, epsilon):
	# TODO: Swap out for the DP library
	series = df[column]
...
# Function takes the output dataframe and either pretty prints an output or writes an output CSV
...
def main():
	...
	df_o = run_aggregations(df_i, parsed.column, parsed.aggregation, parsed.epsilon)
